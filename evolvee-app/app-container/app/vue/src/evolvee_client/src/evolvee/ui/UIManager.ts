...
export default class UIManager {
    ...
    doRequestAskForFriend(userId: number) {
        const { currentUser } = this.game.userManager;
        if (currentUser != null) {
            EvolveeEnvironment.getGame().messenger.requestAddFriend(userId);
        }
    }
    ...
    doRequestDenyFriendRequest(userId: number) {
        const { currentUser } = this.game.userManager;
        if (currentUser != null) {
            EvolveeEnvironment.getGame().messenger.denyFriendRequest(userId);
        }
    }
    doRequestAcceptFriendRequest(userId: number) {
        const { currentUser } = this.game.userManager;
        if (currentUser != null) {
            EvolveeEnvironment.getGame().messenger.acceptFriendRequest(userId);
        }
    }
    doRequestFriendSearch(search: string) {
        const { currentUser } = this.game.userManager;
        if (currentUser != null) {
            EvolveeEnvironment.getGame().messenger.requestFriendSearch(search);
        }
    }
    doRequestRemoveFriend(userId: number) {
        const { currentUser } = this.game.userManager;
        if (currentUser != null) {
            EvolveeEnvironment.getGame().messenger.requestRemoveFriend(userId);
        }
    }
    doRequestFollowFriend(userId: number) {
        const { currentUser } = this.game.userManager;
        if (currentUser != null) {
            EvolveeEnvironment.getGame().messenger.requestFollowFriend(userId);
        }
    }
    doRequestStartChat(userId: number) {
        const { currentUser } = this.game.userManager;
        if (currentUser != null) {
            EvolveeEnvironment.getGame().messenger.requestStartChat(userId);
        }
    }
    ...
    doRequestSendChatMessage(userId: number, text: string) {
        const { currentUser } = this.game.userManager;
        if (currentUser != null) {
            EvolveeEnvironment.getGame().messenger.sendChatMessage(userId, text);
        }
    }
}
