...
export default class ChatManager {
    ...
    addChat(userId: number, message: string) {
        ...
        if (roomUser != null) {
            const image = EvolveeEnvironment.getGame().chatImager.generateChatBubble(0, roomUser.user.name, message, roomUser.avatarContainer.color, roomUser.avatarContainer.headImage);
            const sprite = new Sprite(EvolveeEnvironment.getGame().engine.getTextureFromImage(image));
