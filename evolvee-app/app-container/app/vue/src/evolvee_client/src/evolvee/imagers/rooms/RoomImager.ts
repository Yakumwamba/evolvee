...
export default class RoomImager {
    ...
    initialize() {
        this.roomTileTexture = EvolveeEnvironment.getGame().engine.getTextureFromImage(this.generateFloorTile(7));
        this.roomStairLTexture = EvolveeEnvironment.getGame().engine.getTextureFromImage(this.generateStairL());
        const stairRCanvas = this.generateStairR();
        if (stairRCanvas != null) {
            this.roomStairRTexture = EvolveeEnvironment.getGame().engine.getTextureFromImage(stairRCanvas);
        }
    }

    generateRoomWallL(z: number): Texture {
        return EvolveeEnvironment.getGame().engine.getTextureFromImage(this.generateWallL(122 + (z * 32)));
    }

    generateRoomWallR(z: number): Texture {
        return EvolveeEnvironment.getGame().engine.getTextureFromImage(this.generateWallR(122 + (z * 32)));
    }

    generateRoomDoorL(): Texture {
        return EvolveeEnvironment.getGame().engine.getTextureFromImage(this.generateWallL(28));
    }

    generateRoomDoorBeforeL(z: number): Texture {
        return EvolveeEnvironment.getGame().engine.getTextureFromImage(this.generateWallBeforeDoorL(122 + (z * 32)));
    }
