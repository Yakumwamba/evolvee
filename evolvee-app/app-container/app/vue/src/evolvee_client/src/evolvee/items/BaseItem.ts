...
export default class BaseItem {
    ...
    constructor(furniBase: FurniBase) {
        ...
        this.iconTexture = EvolveeEnvironment.getGame().engine.getTextureFromImage(this.iconImage);
    }
    ...
    _loadTexture(image: HTMLImageElement, key: string) {
        this.textures[key] = EvolveeEnvironment.getGame().engine.getTextureFromImage(image);
        this._loadSolidTexture(image, key);
    }

    _loadSolidTexture(image: HTMLImageElement, key: string) {
        this.solidTextures[key] = EvolveeEnvironment.getGame().engine.getTextureFromImage(generateSilhouette(image, 255, 255, 255));
    }
