...
export default class RoomEngine {
    ...
    centerCamera() {
        ...
        const { currentUser } = EvolveeEnvironment.getGame().userManager;
    ...
    setSelectedTile() {
        const floorTexture = EvolveeEnvironment.getGame().engine.getTexture(ROOM_SELECTED_TILE);
    ...
    setWalls() {
        const { roomImager } = EvolveeEnvironment.getGame();
    ...
    setFloor() {
        const { roomTileTexture, roomStairLTexture, roomStairRTexture } = EvolveeEnvironment.getGame().roomImager;
    ...
    getSelectableColorId(mouseX: number, mouseY: number): number {
        const pixels = EvolveeEnvironment.getGame().engine.logicPixiApp.renderer.extract.pixels(this.getLogicStage());
    ...
    handleMouseClick = (mouseX: number, mouseY: number, shiftKey: boolean, ctrlKey: boolean, altKey: boolean, focusChat: boolean): Selectable | null => {
        ...
        if (isValidTile) {
            EvolveeEnvironment.getGame().communicationManager.sendMessage(new RequestMovement(x, y));
        }
        if (focusChat) {
            EvolveeEnvironment.getGame().uiManager.onFocusChat();
        }
        return selectable;
    }
