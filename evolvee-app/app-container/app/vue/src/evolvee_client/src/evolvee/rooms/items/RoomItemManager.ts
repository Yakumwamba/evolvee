...
export default class RoomItemManager {
    ...
    finishRoomItemMovement(movingItem: RoomItem) {
        if (movingItem === this.currentPlacingItem) {
            this.removeItemFromRoom(movingItem.id, true);
            EvolveeEnvironment.getGame().communicationManager.sendMessage(new RequestFurniPlace(movingItem.id, movingItem._x, movingItem._y, movingItem.rot));
            EvolveeEnvironment.getGame().uiManager.doOpenInventory();
        } else {
            EvolveeEnvironment.getGame().communicationManager.sendMessage(new RequestFurniMove(movingItem.id, movingItem._x, movingItem._y, movingItem.rot));
        }
    }
    ...
    addFloorItemToRoom(id: number, x: number, y: number, z: number, rot: Direction, state: number, baseId: number): Promise<RoomItem> {
        ...
        if (this.currentPlacingItem != null && this.currentPlacingItem.id === id) {
            EvolveeEnvironment.getGame().inventory.tryPlaceBaseItem(baseId);
            this.currentPlacingItem = undefined;
        }
    ...
    removeItemFromRoom(id: number, notifyUi: boolean) {
        ...
        if (item != null) {
            ...
            if (notifyUi) {
                EvolveeEnvironment.getGame().uiManager.onCloseSelectFurni(id);
            }
