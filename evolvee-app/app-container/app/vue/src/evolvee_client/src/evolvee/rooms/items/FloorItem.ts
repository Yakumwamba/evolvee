...
export default class FloorItem extends RoomItem {
    constructor(id: number, x: number, y: number, z: number, rot: Direction, state: number, baseId: number, room: Room) {
        ...
        placeholder.texture = EvolveeEnvironment.getGame().engine.getTexture(FLOOR_ITEM_PLACEHOLDER);
    ...
    rotate() {
        if (this.baseItem != null) {
            EvolveeEnvironment.getGame().communicationManager.sendMessage(new RequestFurniMove(this.id, this._x, this._y, this.baseItem.calculateNextDirection(this.rot)));
        }
    }
}
