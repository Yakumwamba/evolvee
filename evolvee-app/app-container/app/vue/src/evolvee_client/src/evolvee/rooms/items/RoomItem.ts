...
export default abstract class RoomItem implements Selectable {
    ...
    loadBase(): Promise<ContainerGroup> {
        return EvolveeEnvironment.getGame().baseItemManager.getItem(this.getItemType(), this.baseId).then(baseItem => {
    ...
    showItemInfo(isUpdate: boolean) {
        if (this.baseItem != null) {
            ...
            EvolveeEnvironment.getGame().uiManager.onSelectFurni(this.id, this.baseId, this.baseItem.furniBase.itemData.name, this.baseItem.furniBase.itemData.description, this.baseItem.infoImage, isUpdate, true, this.getItemType() === ItemType.FloorItem, true, states !== 1);
        }
    }
    ...
    handleDoubleClick(id: number) {
        EvolveeEnvironment.getGame().communicationManager.sendMessage(new RequestFurniInteract(this.id));
    }

    pickUp() {
        EvolveeEnvironment.getGame().communicationManager.sendMessage(new RequestFurniPickUp(this.id));
    }
}
