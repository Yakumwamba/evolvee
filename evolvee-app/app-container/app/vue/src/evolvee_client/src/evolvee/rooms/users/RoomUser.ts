...
export default class RoomUser implements Selectable {
    ...
    constructor(user: User, x: number, y: number, z: number, rot: Direction, room: Room) {
        ...
        this.shadowSprite = new Sprite(EvolveeEnvironment.getGame().engine.getTexture(ROOM_TILE_SHADOW));
        ...
        const signImage = EvolveeEnvironment.getGame().meMenuImager.generateSign(this.user.name);
        const signTexture = EvolveeEnvironment.getGame().engine.getTextureFromImage(signImage)
    ...
    handleClick = (id: number) => {
        EvolveeEnvironment.getGame().communicationManager.sendMessage(new RequestLookAt(this.user.id));
        this.showUserInfo(false);
    }

    showUserInfo = (isUpdate: boolean) => {
        const isMe = EvolveeEnvironment.getGame().userManager.currentUser === this.user;
        EvolveeEnvironment.getGame().uiManager.onSelectUser(this.user.id, this.user.name, this.user.motto, this.user.look, isMe, this.avatarContainer.userInfoImage, isUpdate);
    }
    ...
    updateTexture() {
        ...
        if (this.loaded) {
            ...
            this.bodySprite.texture = EvolveeEnvironment.getGame().ghostTextures.getBodyTexture(this.rot, action, bodyFrame);
            this.headSprite.texture = EvolveeEnvironment.getGame().ghostTextures.getHeadTexture(this.headRot, gesture, headFrame);
        }
    }
