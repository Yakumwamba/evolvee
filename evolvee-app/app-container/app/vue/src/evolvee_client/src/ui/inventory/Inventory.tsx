...
import UserItem from "../../evolvee/inventory/UserItem";
import { canvas2Image } from "../misc/GraphicsUtilities";
import { ItemType } from "../../evolvee/imagers/furniture/FurniImager";
import { FLOOR_ITEM_PLACEHOLDER } from "../../evolvee/graphics/GenericSprites";
...
> {
  ...
  placeItem = () => {
    ...
    EvolveeEnvironment.getGame().uiManager.doFurniPlace(selectedId);
  };
  ...
  generateGrid(): ReactNode {
    ...
    items
      .filter((item) => currentType === item.itemType)
      .forEach((item) => {
        const count = stackableItems[item.baseId];
        if (
          (item.stackable && stackableItemsDrawn[item.baseId]++ === 0) ||
          !item.stackable
        ) {
  ...
  generateItemInfo(): ReactNode {
    const { selectedId } = this.state;
    const selectedItem =
      EvolveeEnvironment.getGame().inventory.getItem(selectedId);
    ...
    return (
      <div className="item_preview">
        ...
        <div className="item_data">
          ...
          <button onClick={this.placeItem}>Place to room</button>
        </div>
      </div>
    );
  }
      ...
      >
        <div style={{ zIndex }}>
          ...
          <div className="main_tab_container">
            <button className="selected">Furniture</button>
  ...
  generateSecondTab(): ReactNode {
    ...
    return (
      <>
        <button
          onClick={this.toggleType(ItemType.FloorItem)}
          className={floorSelected ? "selected" : ""}
        >
          Floor
        </button>
        <button
          onClick={this.toggleType(ItemType.WallItem)}
          className={!floorSelected ? "selected" : ""}
        >
          Wall
        </button>
