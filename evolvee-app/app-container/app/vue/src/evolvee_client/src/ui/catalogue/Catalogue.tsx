...
import { CatalogueIndex } from "../../evolvee/catalogue/Catalogue";
import CataloguePage from "../../evolvee/catalogue/CataloguePage";
import EvolveeEnvironment from "../../evolvee/EvolveeEnvironment";
import { canvas2Image } from "../misc/GraphicsUtilities";
import WindowManager from "../windows/WindowManager";
import CatalogueItem from "../../evolvee/catalogue/CatalogueItem";
...
const pageColors = [
  4293190884, 4293914607, 0xffffdb54, 4289454682, 4289431551, 4285716709,
  4294016606, 4293326172, 4293694138, 4285383659, 4293082689, 4288782753,
];
...
> {
  ...
  generateTabs(): ReactNode {
    ...
    return pages.map((currentPage) => {
      const children = currentPage.children.map((child) => {
        return (
          <button
            onClick={this.handleChangePage(child.id)}
            key={child.id}
            className={
              "second_tab" + (currentPageId === child.id ? " selected" : "")
            }
          >
            <div className="icon">
              <img
                src={
                  Constants.CATALOGUE_RESOURCES_URL +
                  "icon_" +
                  child.iconId +
                  ".png"
                }
      ...
      const calculatedColor = pageColors[currentPage.color % pageColors.length]
        .toString(16)
        .substr(2);
      ...
      return (
        <Fragment key={currentPage.id}>
          <button
            onClick={this.handleChangePage(currentPage.id)}
            className={
              "main_tab" +
              (currentPageId === currentPage.id ? " selected" : "") +
              (currentTabId === currentPage.id ? " open" : "")
            }
          >
            <div
              className="icon"
  ...
  handleChangePage = (id: number) => () => {
    ...
    EvolveeEnvironment.getGame().uiManager.doRequestCataloguePage(id);
  };
  ...
  generateDescription(): ReactNode {
    ...
    if (currentItemId !== -1) {
      const item = currentPage.items.find(
        (value) => value.itemId === currentItemId
      );
        ...
        return (
          <>
            <div className="image_container">
              <img
                src={image.src}
                alt={item.baseItem.furniBase.itemData.name}
              />
            </div>
    ...
    return (
      <>
        <div className="image_container">
          {currentPage.imageTeaser.length > 0 ? (
            <img
              src={
                Constants.CATALOGUE_RESOURCES_URL +
                "" +
                currentPage.imageTeaser +
                ".gif"
              }
              alt="Furniture"
            />
        ...
        <div className="description_container">{currentPage.textDetails}</div>
      </>
    );
  }
  ...
  handlePurchase = () => {
    ...
    EvolveeEnvironment.getGame().uiManager.doRequestCataloguePurchase(
      currentPageId,
      currentItemId
    );
    this.setState({
  ...
  generateFurniButton(item: CatalogueItem): ReactNode {
    ...
    return (
      <button
        key={item.itemId}
        onClick={this.handleSelectItem(item.itemId)}
        className={currentItemId === item.itemId ? "selected" : ""}
      >
        <img
          ref={ref}
    ...
    return (
      <>
        <div className="frontpage_teaser">
          <img
            alt="border"
            src={Constants.CATALOGUE_RESOURCES_URL + "front_page_border.gif"}
            className="border"
          />
          <img
            alt="article"
            src={
              Constants.TOP_STORY_RESOURCES_URL +
              currentPage.imageTeaser +
              ".gif"
            }
            className="top_story"
          />
          <h2>{currentPage.textHeader}</h2>
          <p>{currentPage.textDetails}</p>
        </div>
  ...
  generateDefaultPage(currentPage: CataloguePage): ReactNode {
    return (
      <>
        <div className="description">{currentPage.textHeader}</div>
        <div className="second_row">
          <div className="grid_container">
            <div className="title">Pick a furni</div>
            <div className="grid">{this.generateGrid()}</div>
          </div>
          <div className="item_description">{this.generateDescription()}</div>
        </div>
      </>
    );
  }
      ...
      return (
        <div className="wrapper">
          <div className="header_container">
            {currentPage.imageHeadline.length > 0 ? (
              <img
                alt="Furniture"
                src={
                  Constants.CATALOGUE_RESOURCES_URL +
                  "" +
                  currentPage.imageHeadline +
                  ".gif"
                }
              />
            ) : (
      ...
      if (item != null && item.baseItem != null) {
        purchaseWindow = (
          <ConfirmPurchase
            zIndex={WindowManager.getNextZIndex()}
            item={item}
            onClose={this.handlePurchaseWindowClose}
        ...
        >
          <div className="catalogue" style={{ zIndex }}>
            ...
            <div className="navigator">
              ...
              <hr />
              <div className="tab_container">{this.generateTabs()}</div>
            </div>
          </div>
        </Draggable>
        {purchaseWindow}
