...
import RoomData, { LockType } from "../../evolvee/navigator/RoomData";
...
> {
  ...
  generateTabs(): ReactNode {
    ...
    return (
      <>
        <button
          onClick={this.handleMainTabChange("rooms")}
          className={mainTabId === "rooms" ? "selected" : ""}
        >
          Rooms
        </button>
        <button
          onClick={this.handleMainTabChange("me")}
          className={mainTabId === "me" ? "selected" : ""}
        >
          Me
        </button>
        <button
          onClick={this.handleMainTabChange("search")}
          className={mainTabId === "search" ? "selected" : ""}
        >
          Search
        </button>
      </>
  ...
  requestRoomList(mainTabId: MainTabId) {
    switch (mainTabId) {
      case "me":
        EvolveeEnvironment.getGame().uiManager.doRequestNavigatorMyRooms();
        break;
      case "rooms":
        EvolveeEnvironment.getGame().uiManager.doRequestNavigatorRooms();
        break;
    }
  }
  ...
  handleMainTabChange = (mainTabId: MainTabId) => () => {
    ...
    if (mainTabId === "search") {
      ...
      if (search.length > 0) {
        EvolveeEnvironment.getGame().uiManager.doRequestNavigatorSearch(search);
      }
    } else {
      this.setState({
        mainTabId,
  ...
  handleGoToRoom = (roomId: number) => () => {
    EvolveeEnvironment.getGame().uiManager.doRequestGoToRoom(roomId);
  };

  handleCreateRoom = () => {
    EvolveeEnvironment.getGame().uiManager.doOpenCreateRoom();
  };
      ...
      .map((roomData) => {
        ...
        return (
          <div
            ...
            key={roomData.id}
            onClick={this.handleGoToRoom(roomData.id)}
          >
            ...
            <div className="icons_container">
              ...
              <button
                className={
                  roomData.isFavourite ? "favourite" : "make_favourite"
                }
              />
              <button
                className={
                  "usercount " +
                  this.calculateUserCountColor(
                    roomData.userCount,
                    roomData.capacity
                  )
                }
    ...
    return (
      ...
      >
        <div style={{ zIndex }}>
          ...
          <hr />
          <div className="main_tab_container">{this.generateTabs()}</div>
          <div className="wrapper">
            ...
            <div className="basic_rooms">{this.generateGrid()}</div>
