...
class FurniInfo extends Component<FurniInfoProps> {
    ...
    useItem = () => {
        ...
        if (itemId !== -1) {
            EvolveeEnvironment.getGame().uiManager.doFurniInteract(itemId);
        }
    }
    ...
    moveItem = () => {
        const { itemId } = this.props;
        if (itemId !== -1) {
            EvolveeEnvironment.getGame().uiManager.doFurniMove(itemId);
        }
    }
    ...
    rotateItem = () => {
        const { itemId } = this.props;
        if (itemId !== -1) {
            EvolveeEnvironment.getGame().uiManager.doFurniRotate(itemId);
        }
    }
    ...
    pickUpItem = () => {
        const { itemId } = this.props;
        if (itemId !== -1) {
            EvolveeEnvironment.getGame().uiManager.doFurniPickUp(itemId);
        }
    }
