...
class RoomInfo extends Component<RoomInfoProps, RoomInfoState> {
  ...
  componentDidMount() {
    EvolveeEnvironment.getGame().uiManager.onCurrentRoomDataLoad = (roomData) => {
      this.setState({
        roomData,
        visible: true,
      });
    ...
    EvolveeEnvironment.getGame().uiManager.onCloseRoomInfo = () => {
      this.setState({
        visible: false,
      });
    };
      ...
      <Draggable>
        <div className="room_info">
          ...
          <p className="owner_info">
            Owner: <span className="owner_name">{roomData.owner}</span>
          </p>
