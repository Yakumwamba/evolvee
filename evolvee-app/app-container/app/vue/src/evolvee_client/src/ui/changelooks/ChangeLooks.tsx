...
} from "../../evolvee/imagers/avatars/AvatarInfo";
...
class ChangeLooks extends React.Component<ChangeLooksProps, ChangeLooksState> {
  ...
  componentDidMount() {
    const game = EvolveeEnvironment.getGame();
  ...
  getMainTabs(): ReactNode {
    ...
    return avatarMainTabs.map((mainTabDef) => {
      return (
        <button
          key={mainTabDef.id}
          onClick={this.handleMainTabChange(mainTabDef.id)}
          className={mainTab === mainTabDef.id ? "selected" : ""}
        >
          <img
            src={"images/avatar_editor/ae_tabs_" + mainTabDef.id + ".png"}
            alt={mainTabDef.id}
          />
        </button>
      );
    ...
    if (mainDef !== undefined) {
      ...
      if (mainDef.id === "generic") {
        ...
        return (
          <>
            <button
              onClick={this.handleToggleGender("M")}
              className={"gender " + (maleSelected ? "selected" : "")}
            >
              <img src={maleImage} alt={maleName} />
              <span>{maleName}</span>
            </button>
            <button
              onClick={this.handleToggleGender("F")}
              className={"gender " + (!maleSelected ? "selected" : "")}
            >
        ...
        return mainDef.tabs.map((secondTabDef) => {
          ...
          return (
            <button
              key={secondTabDef.name}
              onClick={this.handleSecondTabChange(i++)}
  ...
  generatePalette(paletteId: number): ReactNode {
    ...
    if (mainDef !== undefined) {
      ...
      if (typeId != null) {
        const ogPalettes =
          EvolveeEnvironment.getGame().avatarImager.getPartPalette(typeId.type);
        ...
        for (let colorIdStr in ogPalettes) {
          ...
          colors.push(
            <button
              onClick={this.handleChangeColor(paletteId, colorId)}
              key={colorId}
              className={(selected ? "selected " : "") + (isHc ? "hc " : "")}
            >
              <div
                className="palette"
  ...
  generatePalettes(): ReactNode {
    ...
    let colorCount = 1;
    if (currentPart != null) {
      colorCount = EvolveeEnvironment.getGame().avatarImager.getPartPaletteCount(
        currentPart.type,
        currentPart.id
      );
    }
  ...
  generatePartImage(
    ...
    const avatarImager = EvolveeEnvironment.getGame().avatarImager;
    ...
    return (
      <img
        className={selected ? "selected " : ""}
        ref={ref}
        src="images/avatar_editor/avatar_editor_download_icon.png"
        alt={figure}
      />
    );
  }
  ...
  generatePlatform(): ReactNode {
    ...
    avatarImager
      .generateGeneric(
        new AvatarInfo(look, 4, 4, ["std"], "std", 0, false, false, "n"),
        false
      )
      .then((canvas) => {
        const image = canvas2Image(canvas);
  ...
  generateGrid(): ReactNode {
    ...
    if (mainDef !== undefined) {
      const typeId = mainDef.tabs[secondTabId];
      if (typeId != null) {
        const partSet = EvolveeEnvironment.getGame().avatarImager.getPartSet(
          typeId.type
        );
        ...
        if (!typeId.required) {
          parts.push(
            <button
              onClick={this.handleRemovePart(typeId.type)}
              key={-1}
              className={currentPart == null ? "selected " : ""}
            >
              <img
                src="images/avatar_editor/removeSelection.png"
                alt="Remove"
              />
            </button>
          );
        ...
        for (let partId in partSet) {
          ...
          if (
            (part.gender === gender || part.gender === "U") &&
            part.selectable
          ) {
            parts.push(
              <button
                onClick={this.handleChangePart(figure)}
                key={partId}
                className={(selected ? "selected " : "") + (isHc ? "hc " : "")}
              >
  ...
  handleChangePart = (figure: string) => () => {
    const { look } = this.state;
    const newLook = generateFigureString(
      extractFigureParts(look + "." + figure)
    );
  ...
  handleChangeColor = (paletteId: number, colorId: number) => () => {
    ...
    if (currentPart != null) {
      ...
      const newLook = generateFigureString(
        extractFigureParts(look + "." + generateFigureString([currentPart]))
      );
      this.setState({
        look: newLook,
      });
  ...
  handleRemovePart = (type: string) => () => {
    const { look } = this.state;
    const newLook = generateFigureString(
      extractFigureParts(look).filter((value) => value.type !== type)
    );
  ...
  handleSaveChanges = () => {
    ...
    EvolveeEnvironment.getGame().uiManager.doChangeLooks(look, gender);
  };
  ...
  render() {
    ...
    return (
      <Draggable
        defaultClassName="avatar_editor"
        handle=".title"
      ...
      >
        <div style={{ zIndex }}>
          ...
          <h2 className="title">Change your looks</h2>
          <hr />
          <div className="main_tab_container">{this.getMainTabs()}</div>
          <div className="parts_container">
            <div className="second_tab_container">{this.getSecondTab()}</div>
            <div className="first_row">
              <div className="parts_grid">{this.generateGrid()}</div>
              <div className="platform">{this.generatePlatform()}</div>
            </div>
            <div className="palette_container">{this.generatePalettes()}</div>
          </div>
