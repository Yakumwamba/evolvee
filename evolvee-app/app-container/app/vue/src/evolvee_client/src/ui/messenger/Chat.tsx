...
import {
  ...
  MessengerChat,
} from "../../evolvee/messenger/Messenger";
...
export default class Chat extends React.Component<ChatProps, ChatState> {
  ...
  scrollDown() {
    if (this.chatWrapper.current != null) {
      ...
      this.chatWrapper.current.scrollTop =
        this.chatWrapper.current.scrollHeight -
        this.chatWrapper.current.clientHeight;
    }
  }
  ...
  componentDidMount() {
    ...
    uiManager.onReceiveMessengerMessage = (chat: MessengerChat) => {
      const { currentActiveChatId, notifications, activeChats, visible } =
        this.state;
  ...
  handleKeyDown = (evt: any) => {
    ...
    if (isEnter) {
      ...
      if (text.length > 0) {
        EvolveeEnvironment.getGame().uiManager.doRequestSendChatMessage(
          currentActiveChatId,
          text
        );
      }
  ...
  requestFollowFriend = () => {
    ...
    EvolveeEnvironment.getGame().uiManager.doRequestFollowFriend(
      currentActiveChatId
    );
  };
  ...
  requestCloseChat = () => {
    ...
    if (newChats.length === 0) {
      ...
      EvolveeEnvironment.getGame().uiManager.doRequestStartChat(newChats[0].id);
    }
  };
  ...
  generateFriendTab(friend: User) {
    ...
    return (
      <button
        onClick={this.handleTabChange(friend.id)}
        key={friend.id}
        className={className}
      >
        <img
  ...
  generateLeftArrow(): ReactNode {
    ...
    if (currentTabOffset !== 0) {
      return (
        <button
          onClick={this.handleOffsetChange(currentTabOffset === 2 ? -2 : -1)}
          className="arrow"
        >
          <img src="../imagesmessenger/prev.png" alt="" />
        </button>
      );
    }
    return <></>;
  ...
  generateRightArrow(): ReactNode {
    ...
    if (activeChats.length - currentTabOffset !== 6) {
      return (
        <button
          onClick={this.handleOffsetChange(currentTabOffset === 0 ? 2 : 1)}
          className="arrow"
        >
          <img src="../imagesmessenger/next.png" alt="" />
        </button>
      );
    }
    return <></>;
  ...
  generateMainTabs(): ReactNode {
    ...
    if (activeChats.length > 7) {
      ...
      for (let i = 0; i < friendsToShow; i++) {
        friends.push(
          this.generateFriendTab(activeChats[i + Math.max(0, currentTabOffset)])
        );
      }
      return (
        <>
  ...
  render() {
    const { visible, zIndex, text, activeChats, currentActiveChatId } =
      this.state;
    ...
    let title = "Chat";
    const activeFriend = activeChats.find(
      (friend) => friend.id === currentActiveChatId
    );
    ...
    return (
      ...
      >
        <div style={{ zIndex }}>
          ...
          <div className="main_tab_container">{this.generateMainTabs()}</div>
          <div className="actions_container">
            <button onClick={this.requestFollowFriend}>
              <img
                src="../imagesmessenger/follow_friend.png"
                alt="Follow friend"
              />
            </button>
            <button onClick={this.requestCloseChat} className="close_chat">
              <img src="../imagesmessenger/close.png" alt="Close" />
            </button>
          </div>
