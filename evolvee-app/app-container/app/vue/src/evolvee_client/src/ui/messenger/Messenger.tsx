...
import User from "../../evolvee/users/User";
...
> {
  ...
  componentDidMount() {
    const { uiManager } = EvolveeEnvironment.getGame();
  ...
  requestStartChat = (userId: number) => () => {
    EvolveeEnvironment.getGame().uiManager.doRequestStartChat(userId);
  };

  requestFollowFriend = (userId: number) => () => {
    EvolveeEnvironment.getGame().uiManager.doRequestFollowFriend(userId);
  };

  requestAddFriend = (userId: number) => () => {
    EvolveeEnvironment.getGame().uiManager.doRequestAskForFriend(userId);
  };

  requestRemoveFriend = (userId: number) => () => {
    if (userId !== -1) {
      EvolveeEnvironment.getGame().uiManager.doRequestRemoveFriend(userId);
    }
  };

  requestSearch = (search: string) => () => {
    EvolveeEnvironment.getGame().uiManager.doRequestFriendSearch(search);
  };

  requestAcceptFriendRequest = (userId: number) => () => {
    EvolveeEnvironment.getGame().uiManager.doRequestAcceptFriendRequest(userId);
  };

  requestDenyFriendRequest = (userId: number) => () => {
    EvolveeEnvironment.getGame().uiManager.doRequestDenyFriendRequest(userId);
  };
        ...
        <div key={friend.id} className="friend">
          <img
            src={
              ...
              friend.look +
              "&direction=2&head_direction=2&size=s&headonly=1"
            }
            alt={friend.name}
          />
          ...
          <div className="icons_container">
            <button
              onClick={this.requestAcceptFriendRequest(friend.id)}
              className="accept"
            />
            <button
              onClick={this.requestDenyFriendRequest(friend.id)}
              className="decline"
            />
          </div>
        </div>
    ...
    return friends.map((friend) => {
      return (
        <div key={friend.id} className="friend">
          ...
          <div className="icons_container">
            <button
              onClick={this.requestStartChat(friend.id)}
              className="start_chat"
            />
            <button
              onClick={this.requestFollowFriend(friend.id)}
              className="follow_friend"
            />
          </div>
        </div>
      );
    ...
    return friends.map((friend) => {
      return (
        <div key={friend.id} className="friend">
          ...
          <div className="icons_container">
            <button
              onClick={this.requestAddFriend(friend.id)}
              className="ask_for_friend"
            />
          </div>
        </div>
      );
  ...
  generateFriendsTab(): React.ReactNode {
    ...
    if (currentTab === Tabs.Friends) {
      return (
        <>
          <div
            onClick={this.handleChangeTab(Tabs.None)}
            className="main_tab selected"
          >
            <span>Friends</span>
          ...
          <div className="wrapper">
            <div className="friends_container">
              ...
              <div className="friend_list">
                {firstRowActive ? (
                  this.generateOnlineFriendsGrid(onlineFriends)
                ) : (
                  <></>
                )}
              </div>
              ...
              <div className="friend_list">
                {secondRowActive ? (
                  this.generateOfflineFriendsGrid(offlineFriends)
                ) : (
                  <></>
                )}
              </div>
            ...
            <div className="actions_container">
              <button
                onClick={this.requestRemoveFriend(currentSelectedFriendId)}
              >
                <img
                  src="../imagesmessenger/remove_friend.png"
                  alt="Remove friend"
                />
              </button>
              <button onClick={this.requestStartChat(currentSelectedFriendId)}>
                <img src="../imagesmessenger/open_inbox.png" alt="Open inbox" />
              </button>
            </div>
          </div>
        </>
      ...
      return (
        <div
          onClick={this.handleChangeTab(Tabs.Friends)}
          className="main_tab selected"
        >
          <span>Friends</span>
  ...
  generateSearchTab(): React.ReactNode {
    ...
    const filteredFriends = [...onlineFriends, ...offlineFriends].filter(
      (user) => {
        ...
        <>
          ...
          <div className="wrapper search">
            <div className="friends_container">
              ...
              <div className="friend_list">
                {firstRowActive ? (
                  this.generateOfflineFriendsGrid(filteredFriends)
                ) : (
                  <></>
                )}
              </div>
              ...
              <div className="friend_list">
                {secondRowActive ? (
                  this.generateSearchGrid(searchResult)
                ) : (
                  <></>
                )}
              </div>
  ...
  generateRequestsTab(): React.ReactNode {
    ...
    const className =
      "main_tab " + (friendsRequests.length > 0 ? "active" : "");
    ...
    if (currentTab === Tabs.Requests) {
          ...
          <div className="wrapper">
            ...
            <div className="actions_container requests">
              <button onClick={this.requestAcceptAllFriendRequests}>
                <img src="../imagesmessenger/accept.png" alt="Remove friend" />
                <span>Accept all</span>
              </button>
              <button onClick={this.requestDenyAllFriendRequests}>
                <img src="../imagesmessenger/decline.png" alt="Remove friend" />
                <span>Deny all</span>
              </button>
            </div>
          </div>
      ...
      return (
        <>
          <div
            onClick={this.handleChangeTab(Tabs.Requests)}
            className={className}
          >
            <span>Friend requests</span>
            <button className="open_arrow" />
    ...
    return (
      <>
        <div className="footer">
          <button>
            <img src="../imagesmessenger/open_edit_ctgs.png" alt="Settings" />
            <span>Settings</span>
          </button>
        </div>
      </>
  ...
  render() {
    ...
    return (
      <Draggable
        defaultClassName="messenger"
        handle=".title"
        onStart={() => this.upgradeZIndex()}
        onMouseDown={() => this.upgradeZIndex()}
      >
        <div style={{ zIndex }}>
